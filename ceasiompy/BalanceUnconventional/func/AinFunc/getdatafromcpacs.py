    ...
    if fus_nb:
        for i in range(0, fus_nb):
            ...
            if not tixi.checkElement(FUEL_PATH + "/" + F):
                ...
                tixi.updateDoubleElement(FUEL_PATH + "/" + F, ui.F_FUEL[i], "%g")
            else:
...
def get_data(ui, bi, mw, ed, cpacs_in):
    ...
    if not tixi.checkElement(TSPEC_PATH):
        raise Exception(
            "Missing required toolspecific path. Run "
            + "Weight_unc_main.py,"
            + " in the 4Weight_unc_module folder."
        )
    elif not tixi.checkElement(MASS_PATH):
        raise Exception(
            "Missing required massBreakdown path. Run "
            + "Weight_unc_main.py,"
            + " in the 4Weight_unc_module folder."
        )
    elif not tixi.checkElement(MTOM_PATH):
        raise Exception(
            "Missing required mTOM/mass path. Run "
            + "Weight_unc_main.py,"
            + " in the 4Weight_unc_module folder."
        )
    elif not tixi.checkElement(FMP_PATH):
        raise Exception(
            "Missing required fuelMassMaxpass/mass path. Run "
            + "Weight_unc_main.py,"
            + " in the 4Weight_unc_module folder."
        )
    elif not tixi.checkElement(OEM_PATH):
        raise Exception(
            "Missing required mOEM/massDescription/mass "
            + "path. Run Weight_unc_main.py,"
            + " in the 4Weight_unc_module folder."
        )
    elif not tixi.checkElement(PAY_PATH):
        raise Exception(
            "Missing required payload/massDescription/mass "
            + "path. Run Weight_unc_main.py,"
            + " in the 4Weight_unc_module folder."
        )
    elif not tixi.checkElement(F_PATH):
        raise Exception(
            "Missing required /fuel/massDescription/mass "
            + "path. Run Weight_unc_main.py,"
            + " in the 4Weight_unc_module folder."
        )
    elif not tixi.checkElement(EN_PATH):
        raise Exception(
            "Missing required /cpacs/vehicles/engines/engine1"
    ...
    if not tixi.checkElement(GEOM_PATH + "/cabinHeight"):
        ...
        tixi.updateDoubleElement(GEOM_PATH + "/cabinHeight", ui.H_LIM_CABIN, "%g")
    else:
    ...
    if bi.USER_CASE:
        if tixi.checkElement(BC_PATH + "/fuelPercentage"):
            ...
            tixi.updateDoubleElement(BC_PATH + "/fuelPercentage", bi.F_PERC, "%g")
        else:
            raise Exception(
                "User balance option defined"
        ...
        if tixi.checkElement(BC_PATH + "/payloadPercentage"):
            ...
            tixi.updateDoubleElement(BC_PATH + "/payloadPercentage", bi.P_PERC, "%g")
        else:
            raise Exception(
                "User balance option defined"
        ...
        for e in range(0, ed.NE):
            ...
            if tixi.checkElement(ENA_PATH):
                ...
                if e > 0 and ed.en_mass != tp[e - 1]:
                    log.warning(
                        "The engines have different masses,"
    ...
    if bi.USER_EN_PLACEMENT:
        ...
        for e in range(1, ed.NE + 1):
            ...
            if not tixi.checkElement(ENLOC_PATH) and warn:
                raise Exception(
                    "User engine Placement option defined"
