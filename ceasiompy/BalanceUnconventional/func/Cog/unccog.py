...
def unc_center_of_gravity(awg, afg, bout, ui, bi, mw, ed):
    ...
    for i in ui.F_FUEL:
        if i > 80:
            ...
            ui.F_FUEL[fn] = 80
            log.warning('Fuel amount inside the fuselage number ' + str(fn)\
    ...
    (bout.center_of_gravity, mw.mass_seg_i, airplane_centers_segs)\
        = center_of_gravity_evaluation(F_PERC_MAXPASS, 100, afg, awg,\
                                       mw, ed, ui, bi)
    log.info('[x, y, z] = ' + str(bout.center_of_gravity))

    log.info('------- Zero Fuel configuration -------')
    (bout.cg_zfm, mw.ms_zfm, airplane_centers_segs)\
         = center_of_gravity_evaluation(0, 100, afg, awg, mw, ed, ui, bi)
    log.info('[x, y, z] = ' + str(bout.cg_zfm))

    log.info('----- Zero Payload configuration ------')
    (bout.cg_zpm, mw.ms_zpm, airplane_centers_segs)\
        = center_of_gravity_evaluation(100, 0, afg, awg, mw, ed, ui, bi)
    log.info('[x, y, z] = ' + str(bout.cg_zpm))

    log.info('---------- OEM configuration ----------')
    (bout.cg_oem, mw.ms_oem, airplane_centers_segs)\
        = center_of_gravity_evaluation(0, 0, afg, awg, mw, ed, ui, bi)
    ...
    if bi.USER_CASE:
        if bi.P_PERC < 0 or bi.F_PERC < 0:
            raise Exception('Error, F_PERC and P_PERC can'\
                            + ' not be negative.')
        if (mw.mass_fuel_maxpass*(bi.F_PERC/100)\
            + mw.mass_payload*(bi.P_PERC/100)) > mw.mass_fuel_maxpass\
            + mw.mass_payload:
        ...
        (bout.cg_user, mw.ms_user, airplane_centers_segs)\
        = center_of_gravity_evaluation(bi.F_PERC*100, bi.P_PERC, afg, awg, mw,\
                                       ed, ui, bi)
...
def bwb_center_of_gravity(awg, bout, ui, bi, mw, ed):
    ...
    log.info('------ Max Payload configuration ------')
    (bout.center_of_gravity, mw.mass_seg_i, airplane_centers_segs)\
        = center_of_gravity_evaluation_bwb(F_PERC_MAXPASS, 100, awg, mw,\
                                       ed, ui, bi)
    log.info('[x, y, z] = ' + str(bout.center_of_gravity))

    log.info('------- Zero Fuel configuration -------')
    (bout.cg_zfm, mw.ms_zfm, airplane_centers_segs)\
        = center_of_gravity_evaluation_bwb(0, 100, awg, mw, ed, ui, bi)
    log.info('[x, y, z] = ' + str(bout.cg_zfm))

    log.info('----- Zero Payload configuration ------')
    (bout.cg_zpm, mw.ms_zpm, airplane_centers_segs)\
        = center_of_gravity_evaluation_bwb(100, 0, awg, mw, ed, ui, bi)
    log.info('[x, y, z] = ' + str(bout.cg_zpm))

    log.info('---------- OEM configuration ----------')
    (bout.cg_oem, mw.ms_oem, airplane_centers_segs)\
        = center_of_gravity_evaluation_bwb(0, 0, awg, mw, ed, ui, bi)
    log.info('[x, y, z] = ' + str(bout.cg_oem))
    ...
    if bi.USER_CASE:
        ...
        log.info('---------- User configuration ---------')
        (bout.cg_user, mw.ms_user, airplane_centers_segs)\
            = center_of_gravity_evaluation_bwb(bi.F_PERC, bi.P_PERC, awg, mw,\
                                           ed, ui, bi)
        log.info('[x, y, z] = ' + str(bout.cg_user))
