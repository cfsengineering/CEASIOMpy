...
Instead, selecting the 'RANS' option  GMESH will generate only the 2D mesh of the entire aircraft, which will then be processed by the programme Pentagrow, which is included in the [Tetgen](https://wias-berlin.de/software/tetgen/1.5/doc/manual/manual.pdf) package. the hybrid mesh obtained will constitute the 3D domain.
