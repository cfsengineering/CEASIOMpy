...
def engine_conversion(cpacs, engine_uids, brep_dir, engines_cfg_file, engine_surface_percent):
    ...
    reposition_engine(cpacs, closed_engine_path, engine_uids, engines_cfg_file)
...
def close_part(part_path, engine_uids, part_type, engines_cfg_file, engine_surface_percent):
    """
    ...
    # Find the volume with the largest bounding box, to remove the external part of the cylinder
...
def reposition_engine(cpacs, engine_path, engine_uids, engines_cfg_file):
    """
    ...
    # Import the engine in gmsh
    engine_dimtags = gmsh.model.occ.importShapes(str(engine_path), highestDimOnly=True)
    gmsh.model.occ.synchronize()

    # Apply the transformations
    scaling = engine.transf.scaling
    gmsh.model.occ.dilate([engine_dimtags[0]], 0, 0, 0, scaling.x, scaling.y, scaling.z)

    rotation = engine.transf.rotation
    gmsh.model.occ.rotate([engine_dimtags[0]], 0, 0, 0, 1, 0, 0, np.radians(rotation.x))
    gmsh.model.occ.rotate([engine_dimtags[0]], 0, 0, 0, 0, 1, 0, np.radians(rotation.y))
    gmsh.model.occ.rotate([engine_dimtags[0]], 0, 0, 0, 0, 0, 1, np.radians(rotation.z))

    translation = engine.transf.translation
    gmsh.model.occ.translate([engine_dimtags[0]], translation.x, translation.y, translation.z)
    ...
    rotation_new = rz.apply(rotation_new)

    config_file = ConfigFile(engines_cfg_file)

    config_file[f"{engine_uids[0]}_NORMAL_X"] = f"{rotation_new[0]}"
    config_file[f"{engine_uids[0]}_NORMAL_Y"] = f"{rotation_new[1]}"
    config_file[f"{engine_uids[0]}_NORMAL_Z"] = f"{rotation_new[2]}"
    ...
    if engine.sym:
        ...
        config_file[f"{engine_uids[0]}_mirrored_NORMAL_X"] = f"{rotation_new[0]}"
        config_file[f"{engine_uids[0]}_mirrored_NORMAL_Y"] = f"{-rotation_new[1]}"
        config_file[f"{engine_uids[0]}_mirrored_NORMAL_Z"] = f"{rotation_new[2]}"
