    ...
    for i_wing in range(wing_cnt):
        ...
        for i_sec in range(sec_cnt):
            ...
            for i_elem in range(elem_cnt):
                ...
                if integrate_fuselage and np.sqrt((y_LE_abs)**2 + (delta_z)**2) < radius_fus and wg_sec_dihed < math.pi / 2 and root_defined == False:
                ...
                elif integrate_fuselage == False or np.sqrt((y_LE_abs)**2 + (delta_z)**2) > radius_fus or wg_sec_dihed > 0.95 * math.pi / 2:
