...
from ceasiompy.PyTornado import MODULE_NAME, MODULE_DIR
...
def main(cpacs: CPACS, wkdir: Path):
    ...
    settings_from_CPACS = get_pytornado_settings_from_CPACS(cpacs_in)
    ...
    # ===== Setup =====
    shutil.copy(src=cpacs_in, dst=file_pyt_aircraft)

    # ===== Get PyTornado settings =====
    cpacs_settings = get_pytornado_settings(cpacs_in)
    ...
    # ===== Clean up =====
    shutil.copy(src=file_pyt_aircraft, dst=wkdir)
    ...
    while not last_file:
        ...
        wkflow_results_case_dir = Path(wkdir, f"Case_{i:03}")
