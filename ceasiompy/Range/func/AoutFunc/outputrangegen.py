...
def output_txt(LDloi, LDcru, mw, ri, out, NAME):
    ...
    if ri.cabin_crew_nb:
        ...
        OutputTextFile.write("\nTotal crew mass [kg]: " + str(int(round(out.mass_crew))))
        OutputTextFile.write("\nFlight time [min]: " + str(int(round(out.flight_time * 60))))
    OutputTextFile.write("\n-----------------------------------------------")
    OutputTextFile.write("\n-----------------------------------------------")
    OutputTextFile.write("\nFuel Composition ------------------------------")
    OutputTextFile.write(
        "\nTotal fuel with max passengers [kg]: " + str(int(round(mw.mass_fuel_maxpass)))
    )
    OutputTextFile.write("\nFuel for take off [kg]: " + str(int(round(mw.mf_for_to))))
    OutputTextFile.write("\nFuel for climb [kg]: " + str(int(round(mw.mf_for_climb))))
    OutputTextFile.write("\nFuel for cruise [kg]: " + str(int(round(mw.mf_for_cruise))))
    OutputTextFile.write("\nFuel for a 30 min loiter [kg]: " + str(int(round(mw.mf_for_loiter))))
    OutputTextFile.write("\nFuel for landing [kg]: " + str(int(round(mw.mf_for_landing))))
    OutputTextFile.write(
        "\nTotal fuel remaining after landing [kg]: " + str(int(round(mw.mf_after_land)))
    )
    OutputTextFile.write("\n-----------------------------------------------")
    OutputTextFile.write("\nWeigth loss due to fuel usage (max passengers) ")
    OutputTextFile.write(
        "\nTotal fuel storaged with max passengers [kg]: " + str(int(round(mw.mass_fuel_maxpass)))
    )
    OutputTextFile.write("\nWeight after take off [N]: " + str(int(round(mw.w_after_to))))
    OutputTextFile.write("\nWeight after climb [N]: " + str(int(round(mw.w_after_climb))))
    OutputTextFile.write("\nWeight after cruise [N]: " + str(int(round(mw.w_after_cruise))))
    OutputTextFile.write(
        "\nWeight after a 30 min loiter [N]: " + str(int(round(mw.w_after_loiter)))
    )
    OutputTextFile.write("\nWeight after landing [N]: " + str(int(round(mw.w_after_land))))
    OutputTextFile.write("\n-----------------------------------------------")
    OutputTextFile.write("\nRange Analysis --------------------------------")
    OutputTextFile.write("\nTotal range -----------------------------------")
    OutputTextFile.write("\nRange with maximum payload [km]: " + str(int(round(out.ranges[1]))))
    OutputTextFile.write(
        "\nRange with maximum fuel and some payload [km]: " + str(int(round(out.ranges[2])))
    )
    OutputTextFile.write("\nMaximum range [km]: " + str(int(round(out.ranges[-1]))))
    OutputTextFile.write("\nCruise range ----------------------------------")
    OutputTextFile.write(
        "\nCruise range with maximum payload [km]: " + str(int(round(out.ranges_cru[1])))
    )
    OutputTextFile.write(
...
def payload_range_plot(ranges, ranges_cru, payloads, mw, NAME):
    ...
    ax.plot(ranges[1], payloads[1], "ob", label="Range with max payload", markersize=10)
    ...
    if mw.m_pass_middle:
        ax.plot(
            ...
            payloads[2],
            "sr",
            label="Range with max fuel and some payload",
            markersize=10,
        )
    ...
    major_ticks_x = np.arange(
        0, (round(np.amax(ranges), 0) + round(np.amax(ranges), 0) / 4.0), maxr_ax
    )
    minor_ticks_x = np.arange(
        0, (round(np.amax(ranges), 0) + round(np.amax(ranges), 0) / 4.0), minr_ax
    )
    major_ticks_y = np.arange(
        0, (round(mw.mass_payload, 0) + round(mw.mass_payload, 0) / 4.0), maxp_ax
    )
    minor_ticks_y = np.arange(
        0, (round(mw.mass_payload, 0) + round(mw.mass_payload, 0) / 4.0), minp_ax
    )
