...
def fuel_consumption(LDloi, mw, ri):
    ...
    mw.w_after_cruise = mw.w_after_loiter * math.exp(
        ((ri.LOITER_TIME / 60.00) * ri.TSFC_LOITER) / LDloi
    )
    ...
    if mw.w_after_cruise > mw.w_after_climb:
        log.warning(
            "The engine efficiency is too low or payload too"
            + " heavy or not enough fuel to substain the fligth"
        )
        raise Exception(
            "The engine efficiency is too low"
