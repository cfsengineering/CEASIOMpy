...
def breguet_cruise_range(LDcru, ri, mw, RES_FUEL_PERC=0.06):
    ...
    ranges.append(
        round(
            ...
            * LDcru
            * math.log(mw.w_g / mw.w_after_land),
            3,
        )
    )
    ...
    ranges_cru.append(
        round(
            ((ri.CRUISE_SPEED * 3.6) / (ri.TSFC_CRUISE))
            * LDcru
            * math.log(float(mw.w_after_climb) / float(mw.w_after_cruise)),
            3,
        )
    )
    ...
    log.info("Cruise range with maximum payload: " + str(ranges_cru[1]) + " [km]")
    ...
    if m_pass_middle <= 0.0:
        ...
        ranges_cru.append(
            round(
                ((ri.CRUISE_SPEED * 3.6) / (ri.TSFC_CRUISE))
                * LDcru
                * math.log(w_g2_cru / w_al_middle),
                3,
            )
        )
        log.info("Range with maximum fuel and some payload: " + str(ranges[2]) + " [km]")
        log.info(
            "Cruise range with maximum fuel and some payload: " + str(ranges_cru[2]) + " [km]"
        )
    ...
    ranges_cru.append(
        round(
            ((ri.CRUISE_SPEED * 3.6) / (ri.TSFC_CRUISE))
            * LDcru
            * math.log(w_g3_cru / w_al_maxfuel),
            3,
        )
    )
