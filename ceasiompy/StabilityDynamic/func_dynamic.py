...
def check_sign_longi(Cd_alpha,M_w,Cl_alpha,M_dotw,Z_dotw,M_q,Z_q,Cm_eta,Z_eta):
    ...
    if M_w > 0 :
        log.warning('M_w coefficient : Cm_alpha (dcms/daoa) should be negative according to Thomas Yechout Book, P2894')
    if Cl_alpha < 0 :
        log.warning('Z_w coefficient : Cl_alpha (dcl/daoa) should be positive according to Thomas Yechout Book, P289')
    # X_dotw : approx 0
    if M_dotw >0 :
        log.warning('M_wdot coefficient : (Thumb rule), should be negative according to Thomas Yechout Book, P289')
    if Z_dotw > 0 :
        log.warning('Z_wdot coefficient : (Thumb rule), should be negative according to Thomas Yechout Book, P289')
    # cd_q approx 0
    if M_q > 0 :
        log.warning('M_q coefficient : Cm_q (dcmsdqstar), should be negative according to Thomas Yechout Book, P289')
    if Z_q > 0:
        log.warning('Z_q coefficient : -Cl_q (dcldqstar), should be negative according to Thomas Yechout Book, P289')
    # cd_eta approx 0
    if Cm_eta > 0:
        log.warning('M_eta coefficient : Cm_eta (), should be negative according to Thomas Yechout Book, P289')
    if Z_eta > 0:
        log.warning('Z_eta coefficient : -Cl_eta (), should be negative according to Thomas Yechout Book, P289')
...
def check_sign_lat(Y_v,L_v,N_v,Y_p,L_p,Y_r,L_r,N_r,L_xi,Y_zeta,L_zeta,N_zeta):
    ...
    if L_v > 0:
        log.warning('L_v coefficient : Cl_beta (dcmd/daos) should be negative according to Thomas Yechout Book, P304')
    if N_v < 0:
        log.warning('L_v coefficient : Cn_beta (dcml/daos) should be positive according to Thomas Yechout Book, P304')
    if Y_p > 0:
        log.warning('Y_p coefficient : Cy_p (dcsdpstar) should be negative according to Thomas Yechout Book, P304')
    if L_p > 0:
        log.warning('L_p coefficient : Cl_p (dcmddpstar) should be negative according to Thomas Yechout Book, P304')
    # N_p : + or -
    if Y_r < 0:
        log.warning('Y_r coefficient : Cy_r (dcsdrstar) should be positive according to Thomas Yechout Book, P304')
    if L_r < 0:
        log.warning('L_r coefficient : Cl_r (dcmddrstar) should be positive according to Thomas Yechout Book, P304')
    if N_r > 0:
        log.warning('L_r  coefficient : Cn_r (dcmldrstar) should be negative according to Thomas Yechout Book, P304')
    # Y_xi : approx 0
    if L_xi < 0:
        log.warning('L_xi coefficient : Cl_xi () should be positive according to Thomas Yechout Book, P304')
    # N_xi : + or -
    if Y_zeta < 0:
        log.warning('Y_zeta coefficient : Cy_xi () should be positive according to Thomas Yechout Book, P304')
    if L_zeta < 0:
        log.warning('L_zeta coefficient : Cl_xi () should be positive  according to Thomas Yechout Book, P304')
    if N_zeta > 0:
        log.warning('L_zeta  coefficient : Cn_xi () should be negative according to Thomas Yechout Book, P304')
...
def plot_splane(eg_value_longi, plot_title, legend,show_plots, save_plots):
    ...
    plt.plot(eg_value_longi[1].real, eg_value_longi[1].imag,marker='*', color='g', markersize=7,linestyle = 'None')
    plt.plot(eg_value_longi[2].real, eg_value_longi[2].imag, marker='*', color='b', markersize=7,linestyle = 'None')
    plt.plot(eg_value_longi[3].real, eg_value_longi[3].imag,marker='*', color='b', markersize=7,linestyle = 'None')
...
def concise_derivative_longi(X_u,Z_u,M_u,X_w,Z_w,M_w, X_q,Z_q,M_q,X_dotw,Z_dotw,
    ...
    x_q = (mac*X_q - m_adim*We)/m_adim + ((mac*Z_q+m_adim*Ue)*mac/u0*X_dotw)/(m_adim*(m_adim-mac/u0*Z_dotw))
    z_q = (mac*Z_q + m_adim*Ue)/ (m_adim - mac/u0*Z_dotw)
    m_q = (mac*M_q)/i_yy + ((mac*Z_q + m_adim*Ue)*mac/u0*M_dotw)/(i_yy*(m_adim-mac/u0*Z_dotw))

    x_theta = - g*np.cos(theta_e) - (mac/u0*X_dotw*g*np.sin(theta_e))/(m_adim-mac/u0*Z_dotw)   # g is not dimensionlessss!!!!!!!!
    z_theta = - (m_adim*g*np.sin(theta_e))/(m_adim - mac/u0*Z_dotw)                                               # g is not dimensionlessss!!!!!!!!
    m_theta = - (mac/u0*M_dotw*m_adim*g*np.sin(theta_e))/(i_yy*(m_adim - mac/u0*Z_dotw))    # g is not dimensionlessss!!!!!!!!
...
def speed_derivative_at_trim (parameter_list, mach, mach_list, mach_unic_list, idx_alt, aoa_list, aos_list, idx_trim_before, idx_trim_after, ratio):
    ...
    # else,  Mach  value is nor the first nor the last value, of Mach_unic hence make middle derivative
        ...
        idx_mach_in_mach_unic = [j for j in range(len(mach_unic_list)) if mach_unic_list[j] == mach][0] #Find index oh mach in mach_unic_list
