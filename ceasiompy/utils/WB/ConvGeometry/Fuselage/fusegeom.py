...
def check_segment_connection(f_nb, fuse_seg_nb, fuse_sec_nb, tigl):
    ...
    for j in range(2,fuse_seg_nb[f_nb-1]+1):
        (x,y,z) = tigl.fuselageGetPoint(f_nb,j,1.0,0.0)
        if x < slpx:
            (slpx,slpy,slpz) = (x,y,z)
            start_index.append(j)
            seg_sec_reordered[0,f_nb-1,:] = seg_sec[j-1,f_nb-1,:]
    for j in range(2,fuse_seg_nb[f_nb-1]+1):
        end_sec = seg_sec_reordered[j-2,f_nb-1,1]
        start_next = np.where(seg_sec[:,f_nb-1,0]==end_sec)
        seg_sec_reordered[j-1,f_nb-1,:] = seg_sec[start_next[0],:]
    fuse_sec_index.append(seg_sec[0,0,0])
    for j in range(2,fuse_seg_nb[f_nb-1]+1):
         if (seg_sec_reordered[j-1,f_nb-1,0] in fuse_sec_index) == False:
            fuse_sec_index.append(seg_sec_reordered[j-1,f_nb-1,0])
    if (seg_sec_reordered[j-1,f_nb-1,1] in fuse_sec_index) == False:
        fuse_sec_index.append(seg_sec_reordered[j-1,f_nb-1,1])
...
def rel_dist(f_nb, sec_nb, seg_nb, tigl, seg_sec, start_index):
    ...
    for j in range(1,seg_nb+1):
        k = int(seg_sec[j-1,2])
        (slpx2,slpy2,slpz2) = tigl.fuselageGetPoint(f_nb,k,1.0,0.0)
