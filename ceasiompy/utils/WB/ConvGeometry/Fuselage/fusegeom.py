...
def fuse_geom_eval(ag, cpacs_in):
    ...
    (ag.fuse_sec_nb, start_index, seg_sec, fuse_sec_index) = check_segment_connection(
        fus_nb, ag.fuse_seg_nb, ag.fuse_sec_nb, tigl
    )
    ...
    for i in range(int(ag.fuse_nb)):
        ...
        for j in range(1, ag.fuse_seg_nb[i - a - 1] + 1):
            ...
            ag.fuse_center_sec_point[j - 1][i - 1][:] = fuse_center_section_point[j - 1][
                i - a - 1
            ][:]
        if ag.fuse_sym[i - 1 - a] != 0:
            ...
            if ag.fuse_sym[i - 1 - a] == 3:
                ...
                symx = -1
                symz = 1
            ag.fuse_center_seg_point[:, i, 0] = ag.fuse_center_seg_point[:, i - 1, 0] * symx
            ag.fuse_center_seg_point[:, i, 1] = ag.fuse_center_seg_point[:, i - 1, 1] * symy
            ag.fuse_center_seg_point[:, i, 2] = ag.fuse_center_seg_point[:, i - 1, 2] * symz
            ag.fuse_center_sec_point[j - 1][i][:] = fuse_center_section_point[j - 1][i - a - 1][:]
            cs = True
            a += 1
    ...
    tail_length = 0
    for j in range(1, ag.fuse_seg_nb[i - 1] + 1):
        if round(ag.fuse_sec_width[j][i - 1], 3) == round(np.amax(ag.fuse_sec_width[:, i - 1]), 3):
    ...
    while ex is False:
        ...
        cabin_volume = 0
        for j in range(1, ag.fuse_seg_nb[i - 1] + 1):
            if ag.fuse_sec_width[j][i - 1] >= (corr[i - 1] * ag.fuse_mean_width[i - 1]):
                cabin_length += ag.fuse_seg_length[j - 1, i - 1]
                cabin_volume += ag.fuse_seg_vol[j - 1, i - 1]
                cabin_seg[j - 1][i - 1] = 1
                c = True
