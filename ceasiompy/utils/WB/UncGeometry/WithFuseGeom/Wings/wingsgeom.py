...
def wing_geom_eval(w_nb, TP, awg, cpacs_in):
    ...
    awg.wing_sec_thickness = np.zeros((max_wing_sec_nb + 1, awg.w_nb))
    ...
    for i in range(1, awg.w_nb + 1):
        ...
        if (
            ...
            for j in seg_sec[:, i - 1, 2]:
                if j == 0.0:
                    break
                awg.wing_fuel_seg_vol[int(j) - 1][i - 1] = round(
                    (awg.wing_seg_vol[int(j) - 1][i - 1] / (sum(awg.wing_vol)))
                    * awg.wing_fuel_vol,
                    2,
                )
    ...
    log.info(
        "If wing number is greater than 1 the informations of each obj \
