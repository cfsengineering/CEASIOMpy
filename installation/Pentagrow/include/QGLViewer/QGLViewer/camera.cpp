...
Camera::Camera()
  : fieldOfView_(M_PI/4.0f), devicePixelRatio_(1.0f)
{
...
{
  devicePixelRatio_ = camera.devicePixelRatio_;
...
{
  GLint xpix, ypix, hgt;
  xpix = pixel.x()*devicePixelRatio_;
  ypix = pixel.y()*devicePixelRatio_;
  hgt = screenHeight()*devicePixelRatio_;

  float depth;
  // Qt uses upper corner for its origin while GL uses the lower corner,
  // hence the y_opengl = height - 1 - y_Qt
  glReadPixels(xpix, hgt-1-ypix, 1, 1,
               GL_DEPTH_COMPONENT, GL_FLOAT, &depth);
  found = depth < 1.0f;

  return unprojectedCoordinatesOf(Vec(xpix, ypix, depth));
}
...
{
  viewport[0] = 0;
  viewport[1] = screenHeight()*devicePixelRatio_;
  viewport[2] = screenWidth()*devicePixelRatio_;
  viewport[3] = -screenHeight()*devicePixelRatio_;
}
...
Vec Camera::unprojectedCoordinatesOf(const Vec& src, const Frame* frame) const
{
  ...
  // getViewport already accounts for the direction of y in Qt, so src needs
  // to be in Qt pixels, not in OpenGL pixels
  getViewport(viewport);
