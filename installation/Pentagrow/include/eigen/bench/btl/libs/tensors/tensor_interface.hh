...
public :
  ...
  typedef typename eeigen::Tensor<real,2>::Index Index;

  typedef std::vector<real> stl_vector;
  typedef std::vector<stl_vector> stl_matrix;

  typedef eeigen::Tensor<real,2> gene_matrix;
  typedef eeigen::Tensor<real,1> gene_vector;
  ...
  static BTL_DONT_INLINE void matrix_from_stl(gene_matrix & A, stl_matrix & A_stl){
    A.resize(eeigen::array<Index,2>(A_stl[0].size(), A_stl.size()));

    for (unsigned int j=0; j<A_stl.size() ; j++){
      for (unsigned int i=0; i<A_stl[j].size() ; i++){
        A.coeffRef(eeigen::array<Index,2>(i,j)) = A_stl[j][i];
      }
    }
  }
  ...
  static BTL_DONT_INLINE  void matrix_to_stl(gene_matrix & A, stl_matrix & A_stl){
    ...
    for (int j=0;j<N;j++){
      A_stl[j].resize(N);
      for (int i=0;i<N;i++){
        A_stl[j][i] = A.coeff(eeigen::array<Index,2>(i,j));
      }
    }
  }

  static inline void matrix_matrix_product(const gene_matrix & A, const gene_matrix & B, gene_matrix & X, int  /*N*/){
    typedef typename eeigen::Tensor<real_type, 1>::DimensionPair DimPair;
    const eeigen::array<DimPair, 1> dims(DimPair(1, 0));
