...
/* Some notes on eeigen's static assertion mechanism:
 *
...
#define EIGEN_STATIC_ASSERT(CONDITION, MSG)                   \
  {                                                           \
    eeigen::internal::static_assertion<bool(CONDITION)>::MSG; \
  }

#else
  // In some cases clang interprets bool(CONDITION) as function declaration
#define EIGEN_STATIC_ASSERT(CONDITION, MSG)                                  \
  if (eeigen::internal::static_assertion<static_cast<bool>(CONDITION)>::MSG) \
  {                                                                          \
  }
...
#define EIGEN_STATIC_ASSERT_FIXED_SIZE(TYPE)                      \
  EIGEN_STATIC_ASSERT(TYPE::SizeAtCompileTime != eeigen::Dynamic, \
...
#define EIGEN_STATIC_ASSERT_SAME_VECTOR_SIZE(TYPE0, TYPE1)                                                                                                                      \
  EIGEN_STATIC_ASSERT(                                                                                                                                                          \
      (int(TYPE0::SizeAtCompileTime) == eeigen::Dynamic || int(TYPE1::SizeAtCompileTime) == eeigen::Dynamic || int(TYPE0::SizeAtCompileTime) == int(TYPE1::SizeAtCompileTime)), \
      YOU_MIXED_VECTORS_OF_DIFFERENT_SIZES)

#define EIGEN_PREDICATE_SAME_MATRIX_SIZE(TYPE0, TYPE1) \
  (                                                    \
      (int(eeigen::internal::size_of_xpr_at_compile_time<TYPE0>::ret) == 0 && int(eeigen::internal::size_of_xpr_at_compile_time<TYPE1>::ret) == 0) || ((int(TYPE0::RowsAtCompileTime) == eeigen::Dynamic || int(TYPE1::RowsAtCompileTime) == eeigen::Dynamic || int(TYPE0::RowsAtCompileTime) == int(TYPE1::RowsAtCompileTime)) && (int(TYPE0::ColsAtCompileTime) == eeigen::Dynamic || int(TYPE1::ColsAtCompileTime) == eeigen::Dynamic || int(TYPE0::ColsAtCompileTime) == int(TYPE1::ColsAtCompileTime))))
...
#define EIGEN_STATIC_ASSERT_LVALUE(Derived)                        \
  EIGEN_STATIC_ASSERT(eeigen::internal::is_lvalue<Derived>::value, \
                      THIS_EXPRESSION_IS_NOT_A_LVALUE__IT_IS_READ_ONLY)

#define EIGEN_STATIC_ASSERT_ARRAYXPR(Derived)                                                                            \
  EIGEN_STATIC_ASSERT((eeigen::internal::is_same<typename eeigen::internal::traits<Derived>::XprKind, ArrayXpr>::value), \
                      THIS_METHOD_IS_ONLY_FOR_ARRAYS_NOT_MATRICES)

#define EIGEN_STATIC_ASSERT_SAME_XPR_KIND(Derived1, Derived2)                                                   \
  EIGEN_STATIC_ASSERT((eeigen::internal::is_same<typename eeigen::internal::traits<Derived1>::XprKind,          \
                                                 typename eeigen::internal::traits<Derived2>::XprKind>::value), \
