...
template<typename ArrayType> void array(const ArrayType& m)
{
  ...
  VERIFY_IS_APPROX(eeigen::pow(m1,exponents), m1.square());
  VERIFY_IS_APPROX(m1.pow(exponents), m1.square());
  VERIFY_IS_APPROX(eeigen::pow(2*m1,exponents), 4*m1.square());
  VERIFY_IS_APPROX((2*m1).pow(exponents), 4*m1.square());
  VERIFY_IS_APPROX(eeigen::pow(m1,2*exponents), m1.square().square());
  VERIFY_IS_APPROX(m1.pow(2*exponents), m1.square().square());
  VERIFY_IS_APPROX(eeigen::pow(m1(0,0), exponents), ArrayType::Constant(rows,cols,m1(0,0)*m1(0,0)));
...
template<typename ArrayType> void array_real(const ArrayType& m)
{
  ...
  VERIFY((m1.isNaN() == (eeigen::isnan)(m1)).all());
  VERIFY((m1.isInf() == (eeigen::isinf)(m1)).all());
  VERIFY((m1.isFinite() == (eeigen::isfinite)(m1)).all());
  ...
  VERIFY((eeigen::isnan)((m1*0.0)/0.0).all());
  VERIFY((eeigen::isinf)(m4/0.0).all());
  VERIFY(((eeigen::isfinite)(m1) && (!(eeigen::isfinite)(m1*0.0/0.0)) && (!(eeigen::isfinite)(m4/0.0))).all());
...
template<typename ArrayType> void array_complex(const ArrayType& m)
{
  ...
  VERIFY((eeigen::isnan)(m1*zero/zero).all());
#if EIGEN_COMP_MSVC
  // msvc complex division is not robust
  VERIFY((eeigen::isinf)(m4/RealScalar(0)).all());
#else
#if EIGEN_COMP_CLANG
  // clang's complex division is notoriously broken too
  if((numext::isinf)(m4(0,0)/RealScalar(0))) {
#endif
    VERIFY((eeigen::isinf)(m4/zero).all());
#if EIGEN_COMP_CLANG
  }
  else
  {
    VERIFY((eeigen::isinf)(m4.real()/zero.real()).all());
  }
#endif
#endif // MSVC

  VERIFY(((eeigen::isfinite)(m1) && (!(eeigen::isfinite)(m1*zero/zero)) && (!(eeigen::isfinite)(m1/zero))).all());
