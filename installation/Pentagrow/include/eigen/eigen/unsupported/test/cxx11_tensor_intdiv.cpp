...
{
  // Divide by one
  const eeigen::internal::TensorIntDivisor<int32_t, false> div_by_one(1);
  ...
  for (int32_t i = 2; i < 25000; ++i) {
    const eeigen::internal::TensorIntDivisor<int32_t, false> div(i);
  ...
  for (int32_t i = 2; i < 25000; ++i) {
    const eeigen::internal::TensorIntDivisor<int32_t, true> div(i);
...
void test_unsigned_32bit()
{
  for (uint32_t i = 1; i < 25000; ++i) {
    const eeigen::internal::TensorIntDivisor<uint32_t> div(i);
...
void test_signed_64bit()
{
  for (int64_t i = 1; i < 25000; ++i) {
    const eeigen::internal::TensorIntDivisor<int64_t> div(i);
...
void test_unsigned_64bit()
{
  for (uint64_t i = 1; i < 25000; ++i) {
    const eeigen::internal::TensorIntDivisor<uint64_t> div(i);
  ...
  for (int expon = 1; expon < 31; expon++) {
    ...
    for (int num_expon = 0; num_expon < 32; num_expon++) {
      ...
      for (int32_t num = start_num; num < end_num; num++) {
        eeigen::internal::TensorIntDivisor<int32_t> divider =
          eeigen::internal::TensorIntDivisor<int32_t>(div);
  ...
  for (int expon = 0; expon < 63; expon++) {
    ...
    for (int num_expon = 0; num_expon < 63; num_expon++) {
      ...
      for (int64_t num = start_num; num < end_num; num++) {
        eeigen::internal::TensorIntDivisor<int64_t> divider(div);
