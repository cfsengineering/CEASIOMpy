...
void ctms_decompositions()
{
  ...
  // Cholesky module
  eeigen::LLT<Matrix> LLT;
  LLT.compute(A);
  X = LLT.solve(B);
  x = LLT.solve(b);
  eeigen::LDLT<Matrix> LDLT;
  LDLT.compute(A);
  X = LDLT.solve(B);
  x = LDLT.solve(b);

  // Eigenvalues module
  eeigen::HessenbergDecomposition<ComplexMatrix> hessDecomp;
  hessDecomp.compute(complexA);
  eeigen::ComplexSchur<ComplexMatrix> cSchur(size);
  cSchur.compute(complexA);
  eeigen::ComplexEigenSolver<ComplexMatrix> cEigSolver;
  cEigSolver.compute(complexA);
  eeigen::EigenSolver<Matrix> eigSolver;
  eigSolver.compute(A);
  eeigen::SelfAdjointEigenSolver<Matrix> saEigSolver(size);
  saEigSolver.compute(saA);
  eeigen::Tridiagonalization<Matrix> tridiag;
  tridiag.compute(saA);

  // LU module
  eeigen::PartialPivLU<Matrix> ppLU;
  ppLU.compute(A);
  X = ppLU.solve(B);
  x = ppLU.solve(b);
  eeigen::FullPivLU<Matrix> fpLU;
  fpLU.compute(A);
  X = fpLU.solve(B);
  x = fpLU.solve(b);

  // QR module
  eeigen::HouseholderQR<Matrix> hQR;
  hQR.compute(A);
  X = hQR.solve(B);
  x = hQR.solve(b);
  eeigen::ColPivHouseholderQR<Matrix> cpQR;
  cpQR.compute(A);
  X = cpQR.solve(B);
  x = cpQR.solve(b);
  eeigen::FullPivHouseholderQR<Matrix> fpQR;
  fpQR.compute(A);
  // FIXME X = fpQR.solve(B);
  x = fpQR.solve(b);

  // SVD module
  eeigen::JacobiSVD<Matrix> jSVD;
...
void test_reference(const MatrixType &m)
{
  ...
  enum
  {
    Flag = MatrixType::IsRowMajor ? eeigen::RowMajor : eeigen::ColMajor
  };
  enum
  {
    TransposeFlag = !MatrixType::IsRowMajor ? eeigen::RowMajor : eeigen::ColMajor
  };
