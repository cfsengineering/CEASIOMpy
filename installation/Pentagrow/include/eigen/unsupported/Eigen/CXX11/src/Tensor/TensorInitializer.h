...
struct Initializer {
  ...
  static void run(TensorEvaluator<Derived, DefaultDevice>& tensor,
                  eeigen::array<typename traits<Derived>::Index, traits<Derived>::NumDimensions>* indices,
                  const InitList& vals) {
    int i = 0;
    for (auto v : vals) {
...
struct Initializer<Derived, 0> {
  ...
  static void run(TensorEvaluator<Derived, DefaultDevice>& tensor,
                  eeigen::array<typename traits<Derived>::Index, traits<Derived>::NumDimensions>*,
                  const InitList& v) {
    tensor.coeffRef(0) = v;
  }
};
