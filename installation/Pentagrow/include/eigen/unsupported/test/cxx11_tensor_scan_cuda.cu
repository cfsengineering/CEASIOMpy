...
void test_cuda_cumsum(int m_size, int k_size, int n_size)
{
  ...
  eeigen::TensorMap<eeigen::Tensor<float, 3, DataLayout>>
      gpu_t_input(d_t_input, eeigen::array<int, 3>(m_size, k_size, n_size));
  eeigen::TensorMap<eeigen::Tensor<float, 3, DataLayout>>
      gpu_t_result(d_t_result, eeigen::array<int, 3>(m_size, k_size, n_size));
