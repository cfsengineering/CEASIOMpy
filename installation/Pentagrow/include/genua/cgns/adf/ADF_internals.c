	 ...
	 return ;
      ...
      if( link_file[0] != '\0' ) { /* A filename is specified, open it. **/
         ...
         if( ! found ) { /** Not found; try to open it **/
            if (posix_access(link_file,2)) /* check for read-only mode */
               status = "READ_ONLY";
...
if( ADF_file[ top_file_index ] >= 0 ) {
   ADFI_flush_buffers( top_file_index, FLUSH_CLOSE, error_return );
   if( posix_close( ADF_file[ top_file_index ] ) < 0 ) {
      ADF_sys_err = errno;
      *error_return = FILE_CLOSE_ERROR ;
      }
   } /* end if */
...
for( index=0; index < MAXIMUM_FILES; index++ ) {
   if( first_file_in_system[ index ] == top_file_index ) {
      ...
      if( file_in_use[index] && ADF_file[ index ] >= 0 ) {
         ADFI_flush_buffers( index, FLUSH_CLOSE, error_return );
         if( posix_close( ADF_file[ index ] ) < 0 ) {
            ADF_sys_err = errno;
            *error_return = FILE_CLOSE_ERROR ;
            }
	 } /* end if */
...
if( ADFI_stridx_c( status, "READ_ONLY" ) == 0 )
   ...
   FILE *ftmp = tmpfile();
   f_ret = ftmp == NULL ? -1 : posix_fileno(ftmp);
}
else if( ADFI_stridx_c( status, "UNKNOWN" ) == 0 )
   f_ret = file_open( file, f_mode | O_RDWR | O_CREAT, 0666);
else {
        ...
        char *data)
{
   ...
   ADF_sys_err = 0;
   while (bytes_left > 0) {
      nbytes = posix_read (ADF_file[file_index], data_ptr, bytes_left);
      if (0 == nbytes) break;
      if (-1 == nbytes) {
          if (EINTR != errno) {
             ADF_sys_err = errno;
        ...
        const char *data)
{
   ...
   ADF_sys_err = 0;
   while (bytes_left > 0) {
      nbytes = posix_write (ADF_file[file_index], data_ptr, bytes_left);
      if (-1 == nbytes) {
          if (EINTR != errno) {
             ADF_sys_err = errno;
             return -1;
