...
void QGLViewer::setCamera(Camera* const camera)
{
  ...
  camera->devicePixelRatio( this->devicePixelRatio() );
#endif
...
{
#if QT_VERSION >= QT_VERSION_CHECK(5,0,0)
  animationTimerId_ = startTimer(animationPeriod(), Qt::PreciseTimer);
#else
...
void QGLViewer::keyPressEvent(QKeyEvent *e)
{
  ...
  while ((it != end) && ( uint(it.value()) != (key | modifiers)))
    ++it;
...
#if QT_VERSION >= 0x040000
    qWarning("State file created using QGLViewer version %s may not be correctly read.", QGLViewerVersionString().toLatin1().constData());
#else
