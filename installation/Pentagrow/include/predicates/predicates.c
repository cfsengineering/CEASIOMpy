...
/*    robustly performing the orientation and jrsInCircle tests used in */
...
/*    above).                                                                */
/*                                                                           */
/*  Be sure to call jrsExactInit() once, before calling any of the arithmetic */
...
/*                                                                           */
/*    jrsOrient2d(pa, pb, pc) */
/*    jrsOrient2dfast(pa, pb, pc) */
/*    jrsOrient3d(pa, pb, pc, pd) */
/*    jrsOrient3dfast(pa, pb, pc, pd) */
/*    jrsInCircle(pa, pb, pc, pd) */
/*    jrsInCirclefast(pa, pb, pc, pd) */
/*    jrsInSphere(pa, pb, pc, pd, pe) */
/*    jrsInSpherefast(pa, pb, pc, pd, pe) */
/*                                                                           */
...
/* enter -fp:precise mode to enforce 64bit precision doubles */
#ifdef _MSC_VER
...
/*****************************************************************************/
/*                                                                           */
/*  jrsExactInit()   Initialize the variables used for exact arithmetic. */
/*                                                                           */
...
void jrsExactInit(void)
{
  ...
  /* Error bounds for orientation and jrsInCircle tests. */
...
static int fast_expansion_sum_zeroelim(int elen, double *e, int flen, double *f,
                                       double *h)
/* h cannot be e or f. */
{
  double Q;
...
/*****************************************************************************/
/*                                                                           */
/*  jrsOrient2dfast()   Approximate 2D orientation test.  Nonrobust. */
/*  jrsOrient2dexact()   Exact 2D orientation test.  Robust. */
/*  jrsOrient2dslow()   Another exact 2D orientation test.  Robust. */
/*  jrsOrient2d()   Adaptive exact 2D orientation test.  Robust. */
/*                                                                           */
...
/*  result returned is the determinant of a matrix.  In jrsOrient2d() only, */
/*  this determinant is computed adaptively, in the sense that exact         */
/*  arithmetic is used only to the degree it is needed to ensure that the    */
/*  returned value has the correct sign.  Hence, jrsOrient2d() is usually quite
 */
...
static double jrsOrient2dadapt(const double pa[], const double pb[],
...
double jrsOrient2d(const double pa[], const double pb[], const double pc[])
{
  ...
  return jrsOrient2dadapt(pa, pb, pc, detsum);
}

/*****************************************************************************/
/*                                                                           */
/*  jrsOrient3dfast()   Approximate 3D orientation test.  Nonrobust. */
/*  jrsOrient3dexact()   Exact 3D orientation test.  Robust. */
/*  jrsOrient3dslow()   Another exact 3D orientation test.  Robust. */
/*  jrsOrient3d()   Adaptive exact 3D orientation test.  Robust. */
/*                                                                           */
...
/*                                                                           */
/*  The last three use exact arithmetic to ensure a correct answer.  The     */
/*  result returned is the determinant of a matrix.  In jrsOrient3d() only, */
/*  this determinant is computed adaptively, in the sense that exact         */
/*  arithmetic is used only to the degree it is needed to ensure that the    */
/*  returned value has the correct sign.  Hence, jrsOrient3d() is usually quite
 */
...
static double jrsOrient3dadapt(const double pa[], const double pb[],
                   ...
                   const double pd[])
{
  ...
  return jrsOrient3dadapt(pa, pb, pc, pd, permanent);
}
...
/*****************************************************************************/
/*                                                                           */
/*  jrsInCirclefast()   Approximate 2D jrsInCircle test.  Nonrobust. */
/*  jrsInCircleexact()   Exact 2D jrsInCircle test.  Robust. */
/*  jrsInCircleslow()   Another exact 2D jrsInCircle test.  Robust. */
/*  jrsInCircle()   Adaptive exact 2D jrsInCircle test.  Robust. */
/*                                                                           */
...
/*                                                                           */
/*  The last three use exact arithmetic to ensure a correct answer.  The     */
/*  result returned is the determinant of a matrix.  In jrsInCircle() only, */
/*  this determinant is computed adaptively, in the sense that exact         */
/*  arithmetic is used only to the degree it is needed to ensure that the    */
/*  returned value has the correct sign.  Hence, jrsInCircle() is usually quite
 */
...
static double jrsInCircleadapt(const double pa[], const double pb[],
                               const double pc[], const double pd[],
                               double permanent)
{
                   ...
                   double pd[])
{
  ...
  return jrsInCircleadapt(pa, pb, pc, pd, permanent);
}

/*****************************************************************************/
/*                                                                           */
/*  jrsInSpherefast()   Approximate 3D jrsInSphere test.  Nonrobust. */
/*  jrsInSphereexact()   Exact 3D jrsInSphere test.  Robust. */
/*  jrsInSphereslow()   Another exact 3D jrsInSphere test.  Robust. */
/*  jrsInSphere()   Adaptive exact 3D jrsInSphere test.  Robust. */
/*                                                                           */
...
/*               jrsOrient3d()), or the sign of the result will be reversed. */
/*                                                                           */
/*  Only the first and last routine should be used; the middle two are for   */
/*  timings.                                                                 */
/*                                                                           */
/*  The last three use exact arithmetic to ensure a correct answer.  The     */
/*  result returned is the determinant of a matrix.  In jrsInSphere() only, */
/*  this determinant is computed adaptively, in the sense that exact         */
/*  arithmetic is used only to the degree it is needed to ensure that the    */
/*  returned value has the correct sign.  Hence, jrsInSphere() is usually quite
 */
...
static double jrsInSphereexact(const double pa[], const double pb[],
                               const double pc[], const double pd[],
                               const double pe[])
{
...
static double jrsInSphereadapt(const double pa[], const double pb[],
                               const double pc[], const double pd[],
                               const double pe[], double permanent)
{
  ...
  return jrsInSphereexact(pa, pb, pc, pd, pe);
}
                   ...
                   const double pd[], const double pe[])
{
  ...
  return jrsInSphereadapt(pa, pb, pc, pd, pe, permanent);
}

/* Leave -fp:precise mode */
#ifdef _MSC_VER
