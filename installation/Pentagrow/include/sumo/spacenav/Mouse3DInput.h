...
signals:
  ...
  /// signal motion data scaled by driver settings
  void Move3d(std::vector<float>& motionData);

  /// signal motion data as received from device
  void RawMotion3d(short tx, short ty, short tz,
...
private:
  ...
  public:
    ...
    int fTimeToLive; // For telling if the device was unplugged while sending
    bool fIsDirty;
