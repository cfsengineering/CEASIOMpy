...
void PentaGrow::classify()
{
  ...
  for (int i = 0; i < ne; ++i)
  {
    ...
    if (mwall.edge(i).source() == 1019 or mwall.edge(i).target() == 1019)
    {
      cout << "edge " << i << " nbf " << nbf[0] << " " << nbf[1] << " normals ";
      print_vector(nf1);
      print_vector(nf2);
      cout << " cos " << cphi << "(vs ecphi) " << ecphi << endl;
    }
    if (cphi > ecphi)
    {
      etype[i] = Flat;
    ...
    if (i == 3191 or i == 3199)
    {
      cout << " s,t " << s << t << " emid,fmid,nsum ";
      print_vector(emid);
      print_vector(fmid);
      print_vector(nsum);
      cout << " concavity " << ccv << endl;
      cout << "centers ";
      print_vector(f1.center());
      print_vector(f2.center());
      cout << endl;
    }
                              ...
                              int ncrititer, int laplaceiter)
{
  ...
  /*
    cout << "Testing if normal are in y-plane : 3-after lyt smoothing (in GenerateShell)" << endl;
    for (int i = 0; i < nv; ++i)
    {
      cout << "Vertex " << i << " with v ";
      print_vector(mwall.vertex(i));
      cout << " and normal ";
      print_vector(wfntmp[i]);
      cout << " and layer height " << lyt[i] << endl;
    }
    cout << endl;
    */
  ...
  /*
    cout << "Testing if normal are in y-plane : 4-after normal smoothing (in GenerateShell)" << endl;
    for (int i = 0; i < nv; ++i)
    {
      cout << "Vertex " << i << " with v ";
      print_vector(mwall.vertex(i));
