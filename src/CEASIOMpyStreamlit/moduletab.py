...
def add_gui_object(
    ...
    with groups_container[group]:
        ...
        if selected_value == "Run New Simulations":
            ...
            idx = default_value.index(value)
            session_state.xpath_to_update[SMTRAIN_SIMULATION_PURPOSE_XPATH] = (
                f"{key}_simulation_purpose"
            )
            selected_value = st.radio(
                "Choice of simulation Purpose",
                options=default_value,
                index=idx,
                key=f'{key}_simulation_purpose',
                help="choose to train the model for optimal flight conditions given a fixed "
                "geometry or for optimal geometry given a fixed flight conditions.",
                on_change=save_cpacs_file,
            )

            if selected_value == 'Flight Condition Exploration':
                with st.columns([1, 2])[0]:
                    session_state.xpath_to_update[SMTRAIN_NSAMPLES_AEROMAP_XPATH] = (
                        f'{key}_sample_aeromap'
                    )
                    st.number_input(
                        "Number of samples",
                        value=int(get_value_or_default(tixi, SMTRAIN_NSAMPLES_AEROMAP_XPATH, 10)),
                        key=f'{key}_sample_aeromap',
                ...
                for par in AEROMAP_FEATURES:
                    ...
                    session_state.xpath_to_update[xpath + f"/parameter/{par}/min_value/value"] = (
                        f"param_{par}_min"
                    )
                    session_state.xpath_to_update[xpath + f"/parameter/{par}/max_value/value"] = (
                        f"param_{par}_max"
                    )
                        ...
                        if par_selected:
                            with col_2:
                                st.number_input(
                                    ...
                                    format="%0.1f",
                                    key=min_key,
                                    help="Default set to 20% less than the current value."
                            ...
                            with col_3:
                                st.number_input(
                                    ...
                                    format="%0.1f",
                                    key=max_key,
                                    help="Default set to 20% more than the current value. "
                                    "Adjustable as needed.",
                                    on_change=save_cpacs_file,
                                )
            ...
            if selected_value == 'Geometry Exploration':
                ...
                # tixi = st.session_state.cpacs.tixi
                with st.columns([1, 2])[0]:
                    session_state.xpath_to_update[SMTRAIN_NSAMPLES_GEOMETRY_XPATH] = (
                        f'{key}_sample_geom'
                    )
                    st.number_input(
                        "Number of samples",
                        value=int(get_value_or_default(tixi, SMTRAIN_NSAMPLES_GEOMETRY_XPATH, 10)),
                        key=f'{key}_sample_geom',
                        help='Choose the number of samples',
                        on_change=save_cpacs_file
                    )
                ...
                for wing_uid in wings:
                    session_state.xpath_to_update[xpath + f"/{wing_uid}/selected"] = (
                        f"{key}_wing_{wing_uid}"
                    )
                        ...
                        for section_uid in sections:
                            ...
                            with col2:
                                ...
                                section_selected = st.checkbox(
                                    ...
                                    key=sections_key,
                                    help="Select if you want to optimise this section",
                                    on_change=save_cpacs_file,
                                )
                            ...
                            with col3:
                                if section_selected:
                                    ...
                                    for param in WING_PARAMETERS:
                                        section_base = f"{xpath}/{wing_uid}/sections/{section_uid}"

                                        param_local_xpath = (
                                            f"{section_base}/parameters/{param}/status"
                                        )
                                        min_value_path = (
                                            f"{section_base}/parameters/{param}/min_value/value"
                                        )
                                        max_value_path = (
                                            f"{section_base}/parameters/{param}/max_value/value"
                                        )
                                        ...
                                        param_box = st.checkbox(
                                            ...
                                            value=get_value_or_default(tixi, param_local_xpath, 0),
                                            key=param_key,
                                            help="Select if you want to optimise this parameter",
                                            on_change=save_cpacs_file,
                                        )
                                        ...
                                        if param_box:
                                            ...
                                            with col_min:
                                                st.number_input(
                                                    ...
                                                    format="%0.3f",
                                                    key=min_key,
                                                    help="Default set to 20% less than the "
                                                    "current value. Adjustable as needed.",
                                                    on_change=save_cpacs_file,
                                                )
                                            ...
                                            with col_max:
                                                st.number_input(
                                                    ...
                                                    format="%0.3f",
                                                    key=max_key,
                                                    help="Default set to 20% more than the "
                                                    "current value. Adjustable as needed.",
                                                    on_change=save_cpacs_file,
                                                )
        ...
        if selected_value == 'Two levels':
            ...
            session_state.xpath_to_update[SMTRAIN_THRESHOLD_XPATH] = (
                f'{key}_th_rmse'
            )
            with st.columns([1, 2])[0]:
                st.number_input(
            ...
            if selected_mesh_type:
                ...
                with col_size:
                    ...
                    session_state.xpath_to_update[GMSH_MESH_SIZE_FACTOR_FUSELAGE_XPATH] = (
                        f'{key}_fus_msf'
                    )
                    with st.columns([5, 1])[0]:
                        st.number_input(
                            "Fuselage mesh size factor",
                            value=value,
                            format="%0.3f",
                            key=f'{key}_fus_msf',
                            help="Factor proportional to fuselage radius of curvature "
                            "to obtain cell size on it",
                            on_change=save_cpacs_file,
                        )

                    value = get_value_or_default(tixi, GMSH_MESH_SIZE_FACTOR_WINGS_XPATH, 1)
                    session_state.xpath_to_update[GMSH_MESH_SIZE_FACTOR_WINGS_XPATH] = (
                        f'{key}_wing_msf'
                    )
                    with st.columns([5, 1])[0]:
                        st.number_input(
                            "Wings mesh size factor",
                            value=value,
                            format="%0.3f",
                            key=f'{key}_wing_msf',
                            help="Factor proportional to wing radius of curvature "
                            "to obtain cell size on it",
                            on_change=save_cpacs_file,
                        )

                    value = get_value_or_default(tixi, GMSH_MESH_SIZE_ENGINES_XPATH, 0.23)
                    session_state.xpath_to_update[GMSH_MESH_SIZE_ENGINES_XPATH] = (
                        f'{key}_engine_msf'
                    )
                    with st.columns([5, 1])[0]:
                        st.number_input(
                    ...
                    session_state.xpath_to_update[GMSH_MESH_SIZE_PROPELLERS_XPATH] = (
                        f'{key}_propeller_msf'
                    )
                    with st.columns([5, 1])[0]:
                        st.number_input(
                            "Propellers",
                    ...
                    with col_gen1:
                        ...
                        session_state.xpath_to_update[GMSH_N_POWER_FACTOR_XPATH] = (
                            f'{key}_Npower_factor'
                        )
                        with st.columns([5, 1])[0]:
                            st.number_input(
                                "n power factor",
                        ...
                        session_state.xpath_to_update[GMSH_N_POWER_FIELD_XPATH] = (
                            f'{key}_Npower_field'
                        )
                        with st.columns([5, 1])[0]:
                            st.number_input(
                                ...
                                help="Value that changes the measure of fist cells "
                                "near aircraft parts",
                                on_change=save_cpacs_file,
                            )

                        value = get_value_or_default(tixi, GMSH_REFINE_FACTOR_XPATH, 2)
                        session_state.xpath_to_update[GMSH_REFINE_FACTOR_XPATH] = (
                            f'{key}_refine_factor'
                        )
                        with st.columns([5, 1])[0]:
                            st.number_input(
                                "LE/TE refinement factor",
                                value=value,
                                format="%0.3f",
                        ...
                        session_state.xpath_to_update[GMSH_REFINE_TRUNCATED_XPATH] = (
                            f'{key}_refine_trunated_TE'
                        )
                        st.checkbox(
                            'Refine truncated TE',
                        ...
                        session_state.xpath_to_update[GMSH_AUTO_REFINE_XPATH] = (
                            f'{key}_auto_refine'
                        )
                        st.checkbox(
                            'Auto refine',
                            ...
                            help="Automatically refine the mesh on surfaces that are small"
                                "compare to the chosen mesh size, this option increase "
                    ...
                    if selected_mesh_type == 'RANS':
                        with col_gen2:
                            ...
                            session_state.xpath_to_update[
                                GMSH_REFINE_FACTOR_ANGLED_LINES_XPATH
                            ] = f'{key}_refine_angled_factor'

                            with st.columns([5, 1])[0]:
                                st.number_input(
                                    "Refinement factor of lines in between angled surfaces "
                                    "(only in RANS)",
                                    value=value,
                                    format="%0.3f",
                                    key=f'{key}_refine_angled_factor',
                                    help="Refinement factor of edges at intersections that "
                                    "are not flat enough",
                                    on_change=save_cpacs_file,
                                )

                            value = get_value_or_default(tixi, GMSH_NUMBER_LAYER_XPATH, 20)
                            session_state.xpath_to_update[GMSH_NUMBER_LAYER_XPATH] = (
                                f'{key}_n_layer'
                            )
                            with st.columns([5, 1])[0]:
                                st.number_input(
                            ...
                            session_state.xpath_to_update[GMSH_H_FIRST_LAYER_XPATH] = (
                                f'{key}_h_first_layer'
                            )
                            with st.columns([5, 1])[0]:
                                st.number_input(
                                    "Height of first layer [µm]",
                                    value=value,
                                    format="%0.3f",
                                    key=f'{key}_h_first_layer',
                                    help="Is the height of the first prismatic cell,"
                            ...
                            session_state.xpath_to_update[GMSH_MAX_THICKNESS_LAYER_XPATH] = (
                                f'{key}_max_layer_thickness'
                            )
                            with st.columns([5, 1])[0]:
                                st.number_input(
                                    ...
                                    help="The maximum allowed absolute thickness of "
                                    "the prismatic layer.",
                                    on_change=save_cpacs_file,
                                )

                            value = get_value_or_default(tixi, GMSH_GROWTH_RATIO_XPATH, 1.2)
                            session_state.xpath_to_update[GMSH_GROWTH_RATIO_XPATH] = (
                                f'{key}_growth_ratio'
                            )
                            with st.columns([5, 1])[0]:
                                st.number_input(
                                    ...
                                    help="The largest allowed ratio between the wall-normal "
                                    "edge lengths of consecutive cells",
                                    on_change=save_cpacs_file,
                                )

                            value = get_value_or_default(tixi, GMSH_GROWTH_FACTOR_XPATH, 1.4)
                            session_state.xpath_to_update[GMSH_GROWTH_FACTOR_XPATH] = (
                                f'{key}_growth_factor'
                            )
                            with st.columns([5, 1])[0]:
                                st.number_input(
                                    "Growth factor",
                                    ...
                                    help="Desired growth factor between edge lengths "
                                    "of coincident tetrahedra",
                                    on_change=save_cpacs_file,
                                )

                            value = get_value_or_default(tixi, GMSH_FEATURE_ANGLE_XPATH, 40)
                            session_state.xpath_to_update[GMSH_FEATURE_ANGLE_XPATH] = (
                                f'{key}_feature_angle'
                            )
                            with st.columns([5, 1])[0]:
                                st.number_input(
                                    "Feature Angle [grad]",
                                    ...
                                    help="Larger angles are treated as resulting from "
                    ...
                    session_state.xpath_to_update[GMSH_INTAKE_PERCENT_XPATH] = (
                        f'{key}_intake_percent'
                    )
                    with st.columns([5, 1])[0]:
                        st.number_input(
                            ...
                            help="Position of the intake surface boundary condition"
                                "in percentage of the engine length from the beginning "
                                "of the engine",
                            on_change=save_cpacs_file,
                        )

                    value = get_value_or_default(tixi, GMSH_EXHAUST_PERCENT_XPATH, 20)
                    session_state.xpath_to_update[GMSH_EXHAUST_PERCENT_XPATH] = (
                        f'{key}_exhaust_percent'
                    )
                    with st.columns([5, 1])[0]:
                        st.number_input(
                            ...
                            help="Position of the exhaust surface boundary condition in"
                                "percentage of the engine length from the beginning of "
                                "the engine",
                            on_change=save_cpacs_file,
                        )
        if selected_value == 'One levels':
            ...
            st.checkbox(
                ...
                help="Select this model for the simulation (choose more than one "
                "for comparison).",
                on_change=save_cpacs_file,
            )
            ...
            st.checkbox(
                ...
                help="Select this model for the simulation (choose more than one for "
                "comparison).",
                on_change=save_cpacs_file,
            )
