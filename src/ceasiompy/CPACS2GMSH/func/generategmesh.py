...
def _duplicate_disk_actuator_surfaces(part: ModelPart):
    """
...
def _control_disk_actuator_normal():
    """
...
def _import_breps(
    brep_files: List[Path],
    geometry: Union[CPACS, STP],
) -> Tuple[List[ModelPart], List[Tuple[int, int]]]:
    """
    Import brep files into gmsh and return list of ModelPart and parent dimtags.
    Keeps logging and synchronization.
    """
    aircraft_parts: List[ModelPart] = []
    parts_parent_dimtag = []

    log.info(f"Importing files from {brep_files[0].parent if brep_files else 'unknown'}.")
    ...
    return aircraft_parts, parts_parent_dimtag


def _create_external_domain_for_farfield(
    ...
    gmsh.model.occ.synchronize()
    return [(3, farfield)], model_center, domain_length, model_dimensions


def _ensure_domain_is_symmetric(
    model_center: List[float],
    domain_length: float,
    parts_parent_dimtag: List[Tuple[int, int]],
) -> None:
...
def _aircraft_to_farfield(
    ...
    """
    Fragment the aircraft with the farfield and return the final fluid volume.

    The OCC fragment operation returns:
      - fragments_dimtag: list of volume fragments (first entry = full domain)
      - children_dimtag: list of lists of child volumes created from each parent part

    Workflow:
      1. Fragment the external domain with all parent parts to identify child volumes.
      2. If symmetry is used, remove the symmetry-volume children (keep half-domain).
      3. For each imported parent part, keep the child volumes that belong to it and
         discard parents that lost all children due to symmetry.
      4. Detect and remove child volumes shared by multiple parents (these are intersections
         and are not part of the final model).
      5. Associate remaining child entities to their parent parts, then delete the child
         volumes themselves so that only the final domain remains in the model.
      6. Return the final fluid ModelPart corresponding to the remaining volume.

    The association/cleanup ensures that surfaces, lines and points from the original
    parent parts are preserved and assigned correctly to the final domain.
    """
    ...
    if unwanted_parents:
        aircraft_parts = [
            ...
            if part not in unwanted_parents
        ]
...
def _clean_model(
    ...
    inside a fuselage). These entities may have been assigned to parent
    parts during association but should not remain associated to them.
    Compare each part's associated entities to the final domain, remove any
    entities that are not part of that domain, then aggregate the cleaned
    aircraft entities and create the appropriate physical groups / BCs.
    """
...
def generate_gmsh(
    ...
    aircraft_parts, parts_parent_dimtag = _import_breps(
        geometry=geometry,
    ...
    (
        ...
        domain_length,
        model_dimensions,
    ) = _create_external_domain_for_farfield(
        farfield_factor=farfield_factor,
    )

    if symmetry:
        _ensure_domain_is_symmetric(
            model_center=model_center,
            domain_length=domain_length,
            parts_parent_dimtag=parts_parent_dimtag,
        )

    final_domain = _aircraft_to_farfield(
        symmetry=symmetry,
    ...
    aircraft_part_type: List[str] = []
    for part in aircraft_parts:
        aircraft_part_type.append(part.part_type)

    wing_maxlen, fuselage_maxlen = 0.0, 0.0

    if "fuselage" in aircraft_part_type:
        ...
        AIRCRAFT_DICT["fuselage"] = mesh_size_fuselage

    if "wing" in aircraft_part_type or "pylon" in aircraft_part_type:
        ...
        AIRCRAFT_DICT["wing"] = mesh_size_wing
        AIRCRAFT_DICT["pylon"] = mesh_size_wing
    ...
    if rotor_model:
        ...
        for part in aircraft_parts:
            if part.part_type == "rotor":
                _duplicate_disk_actuator_surfaces(part)
