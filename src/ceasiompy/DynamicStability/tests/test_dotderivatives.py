...
from ceasiompy.DynamicStability.func.dotderivatives import (
    ...
    get_main_wing_le,
    scale_coefficients,
    compute_panel_forces,
    access_angle_derivatives,
    compute_velocity_attributes,
...
class TestDotDerivatives(TestCase):
    ...
    def test_check_x_hinge_warns(self):
    ...
    def test_compute_velocity_attributes(self):
        atm = Atmosphere(h=0.0)
        mach = 0.5
        k_alpha = 2.0
        k_beta = 3.0

        omegaalpha, omegabeta, q_dyn = compute_velocity_attributes(atm, mach, k_alpha, k_beta)
        velocity = atm.speed_of_sound[0] * mach

        self.assertAlmostEqual(omegaalpha, k_alpha * velocity)
        self.assertAlmostEqual(omegabeta, k_beta * velocity)
