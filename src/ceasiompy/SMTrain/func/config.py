...
# def select_aeromap
#     aeromap_selected = str(get_value(tixi, SMTRAIN_XPATH_AEROMAP_UID))
#     aeromap_list = cpacs.get_aeromap_uid_list()

#     # if aeromap_selected in aeromap_list:
...
def design_of_experiment(cpacs: CPACS) -> Tuple[int, Dict[str, List[float]]]:
    ...
    for i in range(1,n_param_aeromap+1):
        ...
        if status_param.strip().lower() == "true":
            param_min_value = tixi.getDoubleElement(f"{params_aeromap_xpath}/{name_param}/min_value/value")
            param_max_value = tixi.getDoubleElement(f"{params_aeromap_xpath}/{name_param}/max_value/value")
            range_params_aeromap[name_param] = (param_min_value , param_max_value)
...
def get_elements_to_optimise(cpacs: CPACS):
    ...
    """
    Retrieves the geometric parameters selected by the user for optimisation and the number of sample.
    """
                ...
                if section_status.strip().lower() == "true":
                    ...
                    for k in range(1,n_parameters+1):
                        ...
                        if status_parameter.strip().lower() == "true":
                            parameters_to_optimise.append(f"{wing_name}_{section_name}_{parameter_name}")
                            ...
                            ranges_gui[f"{parameter_name}_of_{section_name}_of_{wing_name}"] = [min_value,max_value]
...
def relative_ranges(cpacs:CPACS, wings_to_optimise: list, sections_to_optimise: list, max_geom_ranges: dict):
    ...
    for wing_selected in wings_to_optimise:
        ...
        for i in range(1,n_sections_wing+1):
            ...
            for params,toll_percentage in max_geom_ranges.items():
                # if section_uid in sections_to_optimise:
                toll = toll_percentage/100
                if params == "twist":
                    node_path = f"{wing_section_path}/section[@uID='{section_uid}']/transformation/rotation/y"
                    val = tixi.getDoubleElement(node_path)
                    key_name = f"{wing_selected}_{section_uid}_{params}"
                    ranges[key_name] = [val * (1-toll), val * (1+toll)]
                elif params == "chord":
                    node_path = f"{wing_section_path}/section[@uID='{section_uid}']/transformation/scaling/x"
                    val = tixi.getDoubleElement(node_path)
                    key_name = f"{wing_selected}_{section_uid}_{params}"
                    ranges[key_name] = [val * (1-toll), val * (1+toll)]
                elif params == "thickness":
                    node_path = f"{wing_section_path}/section[@uID='{section_uid}']/transformation/scaling/z"
                    val = tixi.getDoubleElement(node_path)
                    key_name = f"{wing_selected}_{section_uid}_{params}"
                    ranges[key_name] = [val * (1-toll), val * (1+toll)]
            ...
            for params,toll in max_geom_ranges.items():
                ...
                if any(positioning_uid.startswith(a) for a in sections_to_optimise):
                    if params == "length":
                        node_path = f"{wing_positioning_path}/positioning[@uID='{positioning_uid}']/{params}"
                        val = tixi.getDoubleElement(node_path)
                        key_name = f"{wing_selected}_{section_uid}_{params}"
                        ranges[key_name] = [val * (1-toll), val * (1+toll)]
...
def get_xpath_for_param(tixi : Tixi3, params, wing_uid, section_uid):
    ...
    if params == "twist":
        return f"{WINGS_XPATH}/wing[@uID='{wing_uid}']/sections/section[@uID='{section_uid}']/transformation/rotation/y"
    elif params == "chord":
        return f"{WINGS_XPATH}/wing[@uID='{wing_uid}']/sections/section[@uID='{section_uid}']/transformation/scaling/x"
    elif params == "thickness":
        return f"{WINGS_XPATH}/wing[@uID='{wing_uid}']/sections/section[@uID='{section_uid}']/transformation/scaling/z"
        ...
        if tixi.checkElement(positioning_path):
            ...
            for i in range(count):
                check_pos_uid = tixi.getTextElement(positioning_path + f"/positioning[{i+1}]/toSectionUID")
                if check_pos_uid == section_uid:
                    pos_uid = tixi.getTextAttribute(positioning_path + f"/positioning[{i+1}]", "uID")
                    return f"{WINGS_XPATH}/wing[@uID='{wing_uid}']/positionings/positioning[@uID='{pos_uid}']/{params}"
