...
def get_ted_deflections(tixi, ted_uid):
    ...
    for i_step in range(step_cnt):
        ...
        if hinge_line_rot == 0:
            log.info(
                "A deflection of 0 deg has been found for "
                + ted_uid
...
def get_ted_corner(tixi, tigl, ted_uid):
    ...
    ted_corner = pd.DataFrame({"Edge": [], "Border": [], "Surface": [], "x": [], "y": [], "z": []})
...
def get_hinge_lists(ted_hinge, sym_dir):
    ...
    hinge_list = [
        str(ted_hinge.iloc[0].x),
        str(ted_hinge.iloc[0].y),
        str(ted_hinge.iloc[0].z),
        str(ted_hinge.iloc[1].x),
        str(ted_hinge.iloc[1].y),
        str(ted_hinge.iloc[1].z),
    ]
...
def run_mesh_deformation(tixi, wkdir):
    ...
    nb_proc = get_value_or_default(tixi, SU2_NB_CPU_XPATH, get_sane_max_cpu())
