...
def find_minima(model, x0, bounds, n_runs, target_index):
...
def main() -> None:
    ...
    for x0 in starts_point:
        ...
        minima_all.extend(result)

    with open(csv_path_minima, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(list(params_keys) + ['function_value'])
        for val,x in minima_all:
            writer.writerow(list(x) + [val])

    xpath_dict = {}
    for param in parameters.keys():
        for i in range(1, len(SECTION_UIDS) + 1):
            key = f"{param}_{i}"
            xpath = f"{xpath_coeffs}/positioning[{i}]/{param}"
            xpath_dict[key] = xpath 


    cpacs_template = cpacs_file
    for i, (val,x) in enumerate(minima_all):
        params_dict = {}
        for name, value in zip(params_keys, x):
            params_dict[name] = {"values": [value], "xpath": [xpath_dict[name]]}
        cpacsNewConf_dir = surrogate_model_dir / "NewConfiguration_cpacs" / f"minima_{i+1}.xml"
        update_cpacs(cpacs_template, cpacsNewConf_dir, params_dict)
        print(f"Saved CPACS configuration for minima {i+1} at {cpacsNewConf_dir}")
