...
def POD_shape_func(L,F_ratio):
    ...
    # the POD is modelled as a fuselage with circle profile, due to the fact that it's unknown the correct shape of the POD's surface
    # the shape is composed by three part a quarter chord form 0 to 20% of the length , after a constant region to 50 % of the length and a quarter ellipse untile the tail of the pod. 
    # It is a simple superellipse with the coefficient set to be more closer to the openVSP geompetry.
    r_max = L/F_ratio
    s1 = 0.3
    s2 = 0.6
    x = np.concatenate((
...
def POD_profile(n):
    ...
    # the first and the last point are the nose and the tail of the pod and due to CPACS doesn't accept 
    # point as profile, it is set  default radius and after scale it to zero
    d = 2
