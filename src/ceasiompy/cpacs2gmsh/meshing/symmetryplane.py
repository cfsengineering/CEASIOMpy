...
from ceasiompy.cpacs2gmsh.utility.utils import MeshSettings

from ceasiompy import log


# Functions
def _get_group_by_name(name: str) -> tuple[int | None, list[int]]:
    for _, group_tag in gmsh.model.getPhysicalGroups(dim=2):
        if gmsh.model.getPhysicalName(2, group_tag) == name:
            tags = sorted(gmsh.model.getEntitiesForPhysicalGroup(2, group_tag))
            return group_tag, tags
    return None, []


def _get_symmetry_surfaces_from_line_contact(
    farfield_surface_tags: list[int],
    aircraft_lines: set[int],
) -> list[int]:
    ...
    return symmetry_surface_tags


def _get_symmetry_surfaces_from_planar_fallback(
    farfield_surface_tags: list[int],
) -> list[int]:
    symmetry_surface_tags: list[int] = []
...
def _replace_group(name: str, surface_tags: list[int]) -> None:
    ...
    if surface_tags:
        new_group = gmsh.model.addPhysicalGroup(2, surface_tags)
        gmsh.model.setPhysicalName(2, new_group, name)
...
def generate_symmetry_plane(
    ...
    farfield_group_tag, farfield_surface_tags = _get_group_by_name("Farfield")
    ...
    symmetry_surface_tags = _get_symmetry_surfaces_from_line_contact(
    ...
    if not symmetry_surface_tags:
        symmetry_surface_tags = _get_symmetry_surfaces_from_planar_fallback(
            farfield_surface_tags
        )
    ...
    farfield_group = gmsh.model.addPhysicalGroup(2, new_farfield_surface_tags)
    gmsh.model.setPhysicalName(2, farfield_group, "Farfield")

    _replace_group("symmetry", symmetry_surface_tags)

    wall_group_tag, wall_surface_tags = _get_group_by_name("wall")
