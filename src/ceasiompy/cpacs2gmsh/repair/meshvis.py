...
def analyze_mesh(mesh, name="Mesh", depth=0):
    ...
    try:
        # Basic info
        log.info(f"{indent}Number of points: {mesh.n_points}")
        log.info(f"{indent}Number of cells: {mesh.n_cells}")
        log.info(f"{indent}Mesh type: {type(mesh).__name__}")

        # Detect sharp/non-manifold feature edges (this is not strict self-intersection detection).
        results['intersections'] = mesh.extract_feature_edges(
            boundary_edges=False,
            feature_edges=True,
            manifold_edges=False
        )
        log.info(f"{indent}⚠️ Feature/non-manifold edges: {results['intersections'].n_lines}")
        ...
        if hasattr(mesh, 'compute_cell_quality'):
            ...
            if 'CellQuality' in quality.array_names:
                ...
                log.info(f"{indent}🚨 Degenerate cells: {bad_count}")
                if bad_count > 0:
                    results['has_problems'] = True
            else:
                log.info(f"{indent}ℹ️ No cell quality data available")

    except Exception as e:
        log.info(f"{indent}❌ Analysis error: {str(e)}")
    ...
    try:
        ...
        if results['intersections'] and results['intersections'].n_lines > 0:
            actors['intersections'] = plotter.add_mesh(
                ...
                label=f"Feature/non-manifold edges ({results['intersections'].n_lines})",
                render_lines_as_tubes=True
            )

        # Add y=0 cutting plane if non-empty
        y_plane = mesh.slice(normal=[0,1,0], origin=[0,0,0])
        if y_plane.n_points > 0:
        ...
        log.info(f"❌ Visualization error: {str(e)}")
        plotter.close()
...
def stl_mesh_checker(stl_path: Path) -> None:
    """Analyze and visualize an STL mesh."""
    try:
        main_mesh = pv.read(str(stl_path))

        if isinstance(main_mesh, pv.MultiBlock):
            log.info(f"🔷 File contains MultiBlock structure with {len(main_mesh)} main blocks")
        ...
        log.info(f"❌ Critical error: {str(e)}")


def cgns_mesh_checker(cgns_path: Path) -> None:
    """Backward-compatible alias. Uses STL reader path."""
    stl_mesh_checker(cgns_path)
