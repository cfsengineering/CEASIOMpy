...
# Classes
class TestWingClassification(CEASIOMpyTest):
    def test_detect_normal_profile(self: CEASIOMpyTest) -> None:
        """
    ...
    def test_detect_truncated_profile(self: CEASIOMpyTest) -> None:
        """
        ...
        # Create curve loop for generating the surfaces
        up_curveloop = gmsh.model.occ.addCurveLoop(
            curveTags=[up_bspline_2, te_line_up, up_bspline_1, le_line]
        )
        lo_curveloop = gmsh.model.occ.addCurveLoop(
            curveTags=[lo_bspline_2, te_line_lo, lo_bspline_1, le_line]
        )
        trunc_curveloop = gmsh.model.occ.addCurveLoop(
            [trunc_line_1, te_line_lo, trunc_line_2, te_line_up]
        )
        profile1_curveloop = gmsh.model.occ.addCurveLoop(
            curveTags=[up_bspline_1, trunc_line_1, lo_bspline_1]
        )
        profile2_curveloop = gmsh.model.occ.addCurveLoop(
            curveTags=[up_bspline_2, trunc_line_2, lo_bspline_2]
        )
        gmsh.model.occ.synchronize()
        ...
        for index, line_comp1 in enumerate(lines_composition):
            for line_comp2 in lines_composition[index:]:
                ...
                le_te_pair, found_normal = detect_normal_profile(
                    le_te_pair=le_te_pair,
                    line_comp1=line_comp1,
