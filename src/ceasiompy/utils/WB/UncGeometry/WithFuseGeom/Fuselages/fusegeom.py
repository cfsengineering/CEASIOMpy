...
def check_segment_connection(fus_nb, fuse_seg_nb, fuse_sec_nb, tigl):
    ...
    for i in range(1, fus_nb + 1):
        ...
        for j in range(2, fuse_seg_nb[i - 1] + 1):
            if seg_sec_reordered[j - 1, i - 1, 0] not in fuse_sec_index:
                fuse_sec_index.append(seg_sec_reordered[j - 1, i - 1, 0])
        if seg_sec_reordered[-1, i - 1, 1] not in fuse_sec_index:
...
def fuse_geom_eval(fus_nb, h_min, fuse_thick, F_FUEL, afg, cpacs_in):
    ...
    (afg.fuse_sec_nb, start_index, seg_sec, fuse_sec_index) = check_segment_connection(
        afg.fus_nb, afg.fuse_seg_nb, afg.fuse_sec_nb, tigl
    )
    ...
    for i in range(1, afg.fus_nb + 1):
        (afg.fuse_sec_rel_dist[:, i - 1], afg.fuse_seg_index[:, i - 1]) = rel_dist(
            i,
            afg.fuse_sec_nb[i - 1],
            afg.fuse_seg_nb[i - 1],
            tigl,
        ...
        for j in range(1, afg.fuse_seg_nb[i - 1] + 1):
            ...
            afg.fuse_sec_per[j][i - 1] = tigl.fuselageGetCircumference(i, k, 1.0)
            ...
            for zeta in np.arange(0.0, 1.0, 0.001):
                ...
                if abs(fpz - afg.fuse_center_section_point[j][i - 1][2]) < 0.01:
                    if fpy > afg.fuse_center_section_point[j][i - 1][1] and hw1 == 0.0:
                        ...
                        y1[j, i - 1] = fpy
                        z1[j, i - 1] = fpz
                    elif fpy < afg.fuse_center_section_point[j][i - 1][1] and hw2 == 0.0:
            ...
            for zeta in np.arange(0.0, 1.0, 0.001):
                ...
                if abs(fpy - afg.fuse_center_section_point[j][i - 1][1]) < 0.01:
                    if fpz > afg.fuse_center_section_point[j][i - 1][2] and hh1 == 0.0:
                        hh1 = abs(fpz - afg.fuse_center_section_point[j][i - 1][2])
                    elif fpz < afg.fuse_center_section_point[j][i - 1][2] and hh2 == 0.0:
        ...
        for zeta in np.arange(0.0, 1.0, 0.001):
            ...
            if abs(fpy - afg.fuse_center_section_point[0][i - 1][1]) < 0.01:
                ...
                elif fpz < afg.fuse_center_section_point[0][i - 1][2] and hh2 == 0.0:
    ...
    for i in range(int(afg.fuse_nb)):
        for j in range(1, afg.fuse_seg_nb[i - 1] + 1):
            afg.fuse_center_seg_point[j - 1][i - 1][0] = (
                ...
                + afg.fuse_center_section_point[j][i - 1][0]
            ) / 2
        ...
        if not F_FUEL[i - 1]:
            for j in range(1, afg.fuse_seg_nb[i - 1] + 1):
                if round(afg.fuse_sec_width[j][i - 1], 3) == round(
                    np.amax(afg.fuse_sec_width[:, i - 1]), 3
                ) and (h_min <= afg.fuse_sec_height[j, i - 1]):
            ...
            while ex is False:
                ...
                for j in range(1, afg.fuse_seg_nb[i - 1] + 1):
                    if afg.fuse_sec_width[j][i - 1] >= (
                        corr[i - 1] * afg.fuse_mean_width[i - 1]
                    ) and (h_min <= afg.fuse_sec_height[j, i - 1]):
                        cabin_length += afg.fuse_seg_length[j - 1, i - 1]
                        cabin_volume += afg.fuse_seg_vol[j - 1, i - 1]
                        cabin_seg[j - 1] = 1
    ...
    log.info(
        "Relative distance of each section of each fuselage [m]: \n" + str(afg.fuse_sec_rel_dist)
    )
