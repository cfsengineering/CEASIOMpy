...
def Import_Fuse(Fuselage):
    ...
    for i in range(num_xsecs):
        ...
        # Reorder the profile points according to CPACS requirements
        ...
        if Sections_information[f'Section{n_section_idx}']['spin'] != 0:
            # If a section has non-zero spin, create two additional sections to
            # represent a smooth twisted transition around the original location.
...
def reorder_fuselage_profile(x, y):
    ...
    if len(neg_idx) == 0:
        raise ValueError("No point with negative y-coordinate found")
    ...
    # Divide the profile into two parts: from the trailing edge to the starting point,
    # and from the starting point back to the trailing edge passing through the leading edge
...
def Spin_func(spin, Section_informations, x_loc,x_rot):
    ...
    # np.mod(a, 1.0) wraps any real value of ainto a normalized phase in [0, 1):
    #   a =  0.20 -> phase = 0.20
