...
def get_params_by_name(xsec_id,label):
    # set a parameter that you want and it will give you the value inside the vsp3 file
    result = {}
...
def get_coord_naca4_mod(xsec_id, n):
    ...
    for i, x in enumerate(x_line):
        if x <= thick_loc:
            y_t[i] = (coeffs['a0']*np.sqrt(x) + coeffs['a1']*x + coeffs['a2']*x**2 + coeffs['a3']*x**3)
        else:
            x1 = 1 - x
...
def get_coord_roundedrectangle(xsec_id, n):
    ...
    def make_bezier(p_start, p_corner, p_end, m=20):
        t = np.linspace(0,1,m)
        return (1-t)[:,None]**2*p_start + 2*(1-t)[:,None]*t[:,None]*p_corner + t[:,None]**2*p_end
...
def bezier_curve(ctrl_pts, n_points,s):
    ...
    # If multiple points have the same minimum x, choose the one closest to z=0.
    x_min = np.min(curve[:, 0])
    tol = 1e-10
    le_candidates = np.where(np.abs(curve[:, 0] - x_min) <= tol)[0]
    if len(le_candidates) == 0:
...
def spline_curve(ctrl_pts, n_points,s):
    ...
    for i in range(n_segments):
        ...
        # Avoid duplicating knot points between consecutive segments.
        t = np.linspace(t1, t2, n_points_per_seg, endpoint=(i == n_segments - 1))
        for tt in t:
    ...
    # Remove residual duplicate consecutive points
    curve_diff = np.diff(curve, axis=0)
    curve = curve[np.concatenate([[True], np.any(np.abs(curve_diff) > 1e-12, axis=1)])]
    ...
    plt.plot(lower_surface[-1,0],lower_surface[-1,1],'yo',label='LE_lower')
    plt.plot(upper_surface[0,0],upper_surface[0,1],'ro',label='LE_upper')
    plt.plot(upper_surface[-1,0],upper_surface[-1,1],'go',label='TE_upper')
    plt.legend()
    plt.title('Lower Surface')
    plt.axis('equal')
    plt.savefig('lower_surface.png')
    # --- Combine surfaces without duplicating TE or LE ---
    ordered_curve = np.vstack([lower_surface, upper_surface])
    ...
    # Enforce exactly n_points in output
    ...
    if len(x) != target_n:
        ...
        y = y[keep]
        if len(x) > 1:
            s_arc = np.concatenate([[0.0], np.cumsum(np.hypot(np.diff(x), np.diff(y)))])
            if s_arc[-1] > 0:
                s_new = np.linspace(0.0, s_arc[-1], target_n)
                x = np.interp(s_new, s_arc, x)
                y = np.interp(s_new, s_arc, y)
            else:
...
def linear_curve(ctrl_pts, n_points,s):
    'import an airfoil file from the Selig database'
...
def Tesselation(Component,idx, x, y):
    ...
    if vsp.GetXSecShape(idx) in [7,15,16,17]:
        print('Tesselation ...')
        ...
        f_u = interp1d(x_upper, y_upper, kind='cubic',
